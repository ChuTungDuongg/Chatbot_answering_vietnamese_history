...
from app.agents.central.citations import check_citations, expand_citations, citation_display_map
from app.agents.central.evidence import SynthesisEvidence, build_evidence_packet, render_evidence_packet, select_evidence, select_synthesis_evidence
from app.agents.central.analytical import annotate_evidence, coverage_report, coverage_select, evidence_targets, strong_evidence
from app.agents.central.semantics import target_mentions, normalize_entity
from app.agents.central.compaction import compact_history
from app.agents.central.evidence import evidence_plan
from app.agents.central.depth import depth_contract, answer_coverage
from app.agents.central.repair import issue_fingerprint, issue_count, remove_optional_viewpoint
from app.agents.central.viewpoints import viewpoint_repair_plan
from app.agents.central.citation_recovery import PURE_CITATION_ISSUES, align_citations, apply_citation_mapping, citation_repair_messages
from app.agents.central.grounding import grounding_risks, entity_alias_matches
from app.agents.central.relationships import (RELATION_CAVEAT, has_relation_caveat, relationship_answer_issues,
                                             relationship_contract, annotate_relationship)
from app.agents.central.model_runtime import CentralGeneration, CentralLLMBackend, CentralToolCall
from app.agents.central.policy import CentralRequestPolicy, HistoryGroundingPolicy
from app.agents.central.prompt import BIOGRAPHY_CONTRACT, CENTRAL_SYSTEM_PROMPT, REPAIR_CONTRACT, SYNTHESIS_CONTRACT, COMPARISON_CONTRACT, VIEWPOINT_CONTRACT
from app.agents.central.question import analytical_answer_issues, analyze_central_question, plan_analytical_queries
from app.agents.central.targets import resolve_comparison_targets, canonical_for
from app.agents.central.administration import administrative_contract, administrative_answer_issues, LEVEL_NAMES
from app.agents.central.facets import (multi_facet, facet_contract, facet_answer_issues, facet_limitation,
                                      FACET_QUERY)
...
class CentralAgent:
    ...
    async def _recover_filter_collapse(self, state, context, progress):
        ...
        target = target or analysis.canonical_target or analysis.event or analysis.subject
        ...
        call = CentralToolCall("central_filter_recovery", "search_history", {"query": analysis.canonical_target or canonical_for(analysis, target), "top_k": self._retrieval_limit(state, "search_history")})
    ...
    async def _fetch_wikipedia_results(self, state: CentralAgentState, context: ToolExecutionContext, progress) -> None:
        ...
        for key, rows in grouped.items():
            ...
            target = key if key in state.question_analysis.comparison_targets else state.question_analysis.canonical_target or state.question_analysis.event or state.question_analysis.subject
    ...
    def _check_answer(self, state: CentralAgentState, packet: list[SynthesisEvidence], *, stage: str):
        ...
        if multi_facet(state.question_analysis) and state.final_answer.strip():
            for facet in state.evidence_debug.get("unresolved_facets", []):
                ...
                if limitation not in state.final_answer:
                    state.final_answer = state.final_answer.rstrip() + "\n\n" + limitation
        citations = check_citations(state.final_answer, packet, state.question_analysis)
        ...
        if citations.target_mismatches:
            issues.append("relational_citation_target_mismatch" if state.question_analysis.event_type else "comparison_citation_target_mismatch")
        ...
        state.evidence_debug.update({
            ...
            "citation_paragraph_classifications": citations.paragraph_classifications,
