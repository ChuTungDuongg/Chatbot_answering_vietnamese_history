...
from app.agents.central.semantics import ACTOR_ALIASES, CentralQuestionAnalysis, _ascii_fold_vietnamese, normalized_actor_text
...
def analyze_central_question(question: str) -> CentralQuestionAnalysis:
    ...
    from app.agents.central.administration import administrative_question
    from app.agents.central.targets import resolve_comparison_targets
    admin = administrative_question(question) if not targets and question_type != "biography" else {}
    from app.agents.central.events import extract_relational_event
    relational = extract_relational_event(question) if not targets and question_type != "biography" and not admin else {}
    if relational and not relational["actors"] and outcome and (event or analytical_subject):
        # A noun in an existing subject (e.g. an alliance) is not a new predicate.
        relational = {}
    if relational:
        event, actors = relational["event"], relational["actors"]
    from app.agents.central.depth import answer_depth
    facets = ("identity", "relationship") if relation else extract_requested_facets(question, question_type)
    if relational:
        # Event vocabulary (e.g. diplomatic relations) is not a requested facet.
        facet_clause = re.search(r"\b(?:xet ve|ve mat|tren phuong dien|ve)\s+(.+)$", normalized)
        facets = extract_requested_facets(facet_clause.group(1) if facet_clause else "", question_type)
    return resolve_comparison_targets(CentralQuestionAnalysis(
        ...
        event_type=relational.get("event_type"), canonical_target=relational.get("canonical_target"),
        actors=actors if question_type != "biography" else (),
        outcome=outcome if question_type != "biography" else None,
        facets=facets,
        answer_depth=answer_depth(question_type, event=None if relational else event, subject=analytical_subject,
...
def plan_analytical_queries(analysis: CentralQuestionAnalysis, max_variants: int = 2) -> dict[str, list[str]]:
    ...
    if analysis.comparison_targets:
        ...
        from app.agents.central.targets import canonical_for
    ...
    from app.agents.central.facets import multi_facet, analytical_facets, FACET_QUERY
    if analysis.canonical_target and analysis.event_type:
        facets = " ".join(FACET_QUERY.get(facet, facet) for facet in analysis.facets)
        return {analysis.canonical_target: list(dict.fromkeys([
            f"{analysis.canonical_target} {facets}".strip(), analysis.question,
        ]))[:max_variants]}
