...
from app.agents.central.semantics import BRACKET_RE

import re
from dataclasses import dataclass, field

from app.agents.central.evidence import SynthesisEvidence
from app.agents.central.semantics import _ascii_fold_vietnamese
from app.agents.central.semantics import target_mentions
from app.agents.central.viewpoints import viewpoint_attribution_issues
...
class CitationCheck:
    ...
    paragraph_classifications: list[dict] = field(default_factory=list)


def check_citations(answer: str, packet: list[SynthesisEvidence], analysis=None) -> CitationCheck:
    ...
    for paragraph_index, paragraph in enumerate(paragraphs):
        ...
        if not cited_aliases:
            from app.agents.central.citation_support import supported_synthesis_summary
            summary = supported_synthesis_summary(paragraph, paragraphs[max(0, paragraph_index - 3):paragraph_index], packet, analysis)
            classifications.append({"paragraph": paragraph_index + 1,
                                    "kind": "supported_synthesis_summary" if summary else "new_factual_claim"})
            if not summary:
                uncited += 1
        ...
        if cited_aliases:
            ...
            if analysis and analysis.event_type:
                from app.agents.central.events import event_matches, relational_target_features
                from app.agents.central.depth import actor_scope
                if event_matches(plain, analysis.event) or len(actor_scope(plain, analysis.actors)) > 1:
                    if not any(relational_target_features({"title": evidence_by_alias[alias].title,
                                                          "text": evidence_by_alias[alias].text}, analysis)["direct_target_coverage"]
                               for alias in cited_aliases):
                        mismatches.append(analysis.canonical_target or analysis.event)
    ...
    return CitationCheck(
        ...
        normalized_answer != answer, uncited, list(dict.fromkeys(mismatches)), unattributed, viewpoint_issues, cross_support, classifications,
    )
