...
from app.agents.central.analytical import DIMENSION_CUES, cause_sentence_relevance
from app.agents.central.semantics import normalize_entity
from app.agents.central.question import analyze_central_question
from app.agents.central.facets import neutral_preference, viewpoint_cost, evidence_facets, multi_facet
...
def excerpt_evidence(text: str, analysis, limit: int) -> str:
    ...
    def score(value):
        ...
        if analysis.administrative_level:
            from app.agents.central.administration import annotate_administration
        ...
        base = len(words & query) + 3 * len(dims & requested) + len(dims) + cause + administrative + neutral + facet_gain
        if analysis.event_type:
            from app.agents.central.events import relational_target_features
            target = relational_target_features({"text": value}, analysis)
            return (target["target_consistency_score"], cause, base)
        return (0, 0, base)
    ranked = sorted(range(len(units)), key=lambda i: score(units[i]), reverse=True)
    chosen: set[int] = set()
    window_count = 0
