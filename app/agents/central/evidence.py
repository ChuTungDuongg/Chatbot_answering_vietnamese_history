...
from app.agents.central.depth import dimension_spans
from app.agents.central.semantics import CentralQuestionAnalysis, normalize_entity
from app.agents.central.viewpoints import annotate_viewpoints
from app.agents.central.compaction import excerpt_evidence
from app.agents.central.relationships import select_relationship_evidence, mentions
from app.agents.central.entity_aliases import evidence_aliases
from app.agents.central.facets import multi_facet
from app.chat.attachments import is_attachment_question
from app.agents.central.config import CentralAgentConfig
from app.agents.central.analytical import annotate_evidence, coverage_select, evidence_targets, strong_evidence, source_role
...
def select_evidence(
    ...
    *,
    ...
    if (analysis.event_type or analysis.question_type in {"cause", "comparison"} or multi_facet(analysis)) and (target or analysis.event or analysis.subject):
        ...
        reasons = Counter(row["entity_filter_reason"] or "analytical_target_mismatch"
                          for row in annotated if not row["target_consistent"] and not row["target_match_uncertain"])
        kept = [row for row in annotated if row["target_consistent"] or row["target_match_uncertain"]]
        ...
        return kept, {
            ...
            "retrieval_downrank_reasons": {"target_match_unconfirmed": sum(row["target_match_uncertain"] for row in kept)},
...
def select_synthesis_evidence(rows: list[dict[str, Any]], analysis: CentralQuestionAnalysis, config: CentralAgentConfig) -> list[dict[str, Any]]:
    # Score the same excerpt budget that can fit the final packet, before ranking.
    preview_chars = min(config.evidence_excerpt_chars, max(0, config.synthesis_char_budget // max(1, min(len(rows), config.analytical_max_sources)) - 200))
    if analysis.relation_requested:
        ...
        if analysis.event_type or analysis.answer_depth == "broad_analysis" or analysis.question_type == "cause" or multi_facet(analysis):
            ...
            selected = [annotate_evidence({**row,
                "text": excerpt_evidence(str(row.get("text") or ""), analysis, preview_chars),
        ...
        if analysis.event_type or analysis.question_type in {"cause", "significance", "consequence", "evaluation"} or multi_facet(analysis):
            ...
            if len(strong) >= 3 and not multi_facet(analysis):
                selected = strong
            selected = coverage_select(selected, analysis, config.analytical_max_sources, min_chars=config.strong_evidence_min_chars)
        else:
    ...
    for row in selected:
        ...
        item = annotate_evidence(item, analysis)
        # Unconfirmed candidates remain available for recovery, but are not
        # model-visible evidence until their target association is established.
        if item.get("target_match_uncertain"):
            continue
        bounded.append(item)
    return bounded
...
class SynthesisEvidence:
    ...
    source_role: str = "primary_factual"
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, "viewpoint_sensitive", any(a["requires_attribution"] for a in annotations))
        object.__setattr__(self, "source_role", source_role({"title": self.title}, self.viewpoint_sensitive))
...
def build_evidence_packet(sources: list[dict[str, Any]]) -> list[SynthesisEvidence]:
    ...
    for row in sources:
        ...
        packet.append(SynthesisEvidence(
            ...
            title=str(row.get("relational_source_title") or row.get("title") or ""), source_kind=str(row.get("source_kind") or "history"), text=text,
...
def render_evidence_packet(packet: list[SynthesisEvidence]) -> str:
    ...
    def render(item):
        ...
        return f"[{item.alias}]\ntitle: {item.title}\nsource_kind: {item.source_kind}\nsource_role: {item.source_role}{note}\ntext: {item.text}"
...
def evidence_plan(packet, analysis, config):
    from app.agents.central.analytical import annotate_evidence, strong_evidence
    sources = {}
    covered_actors = set()
    for item in packet:
        row = annotate_evidence({"title": item.title, "text": item.text}, analysis)
        covered_actors.update(row["actor_scope"])
        if not strong_evidence(row, min_chars=config.strong_evidence_min_chars):
            continue
        for dimension in dimension_spans(item.text, analysis.event if analysis.event_type else None):
