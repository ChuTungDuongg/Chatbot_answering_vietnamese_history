...
from app.agents.central.semantics import CentralQuestionAnalysis, _ascii_fold_vietnamese, normalize_entity, target_key, target_mentions
from app.agents.central.viewpoints import annotate_viewpoints
from app.agents.central.relationships import annotate_relationship, relationship_coverage
from app.agents.central.targets import canonical_for, entity_type_consistent
from app.agents.central.administration import annotate_administration, administrative_coverage
from app.agents.central.facets import evidence_facets, facet_coverage, multi_facet, neutral_preference, viewpoint_cost
from app.agents.central.depth import dimension_spans, actor_scope
from app.agents.central.events import relational_target_features
...
_CAUSE_LINK = re.compile(r"\b(?:nguyen nhan|nho|boi vi|gop phan|dan den|khien|tao dieu kien|lien quan den|tac dong den|lam suy yeu|lam giam|nham|muc tieu|loi ich|dong co|thuc day|suc ep|chu truong|because|due to|aimed to|sought to|incentive|constraint|policy shift|strategic consideration)\b|\b(?:thanh cong|thang loi|that bai|suy yeu|sup do)\b.{0,45}\b(?:do|vi|boi)\b")
...
def cause_sentence_relevance(text: str) -> int:
    ...
    if _CAUSE_DISTRACTIONS.search(folded):
        return -1
    if _CAUSE_LINK.search(folded) or re.search(r"\b(?:vì|do|nhờ)\s+\w", text, re.I):
        return 2
...
def entity_consistency(row: dict[str, Any], target: str, *, person: bool = False) -> tuple[bool, bool, str | None]:
    ...
    if not person:
        ...
        if title and not associated:
            from app.agents.central.targets import entity_head
...
def annotate_evidence(row: dict[str, Any], analysis: CentralQuestionAnalysis, target: str | None = None) -> dict[str, Any]:
    ...
    row.update({
        ...
        "direct_target_coverage": bool(associated and (overview or target and any(
            target_mentions(s, target) and cause_sentence_relevance(s) > 0 for s in re.split(r"[.!?;\n]", text)))),
    })
    row["target_consistency_score"] = 3 if row["direct_target_coverage"] else 1 if row["target_consistent"] else 0
    ...
    elif analysis.event_type:
        row.update(relational_target_features(row, analysis))
        # An action/date is chronology, not an explanation of the action.
        row["causal_relevance"] = cause_score >= 2
        row["causal_target_required"] = analysis.question_type == "cause"
    row["source_role"] = source_role(row, row["viewpoint_sensitive"])
    row["strong_evidence_dimensions"] = sorted(dimension_spans(text, analysis.event if analysis.event_type else None)) if row["target_consistent"] and not chronology and not cause_downranked else []
...
def coverage_select(rows: list[dict[str, Any]], analysis: CentralQuestionAnalysis, limit: int, *, min_chars: int = 100) -> list[dict[str, Any]]:
    """Target and facet relevance precede marginal coverage; ranks are batch-local."""
    pending = list(rows)
    ...
    while pending and len(selected) < limit:
        def priority(row):
            ...
            if analysis.event_type:
                # A direct joint source never loses to a different relation merely
                # because that source supplies a previously uncovered dimension.
                return (
                    row.get("target_consistency_score", 0),
                    row.get("cause_facet_score", 0) if analysis.question_type == "cause" else len(set(row.get("evidence_facets", [])) & requested),
                    bool(row.get("citable", True)),
                    not row.get("viewpoint_sensitive", False),
                    -int(row.get("retrieval_rank", 999)),
                    bool(row.get("overview_anchor")),
                    len(set(row.get("strong_evidence_dimensions", [])) - covered),
                )
            if analysis.answer_depth == "broad_analysis" and neutral_preference(analysis):
                ...
                return (
                    row.get("target_consistency_score", 0) if analysis.event else 0,
            ...
            neutral_alternative = neutral_preference(analysis) and row.get("viewpoint_cost", 0) > .25 and any(
                ...
                for other in pending if other is not row)
            return (
                row.get("target_consistency_score", 0) if analysis.event and not analysis.comparison_targets and not analysis.administrative_level else 0,
...
def strong_evidence(row: dict[str, Any], *, min_chars: int) -> bool:
    ...
    return bool(row.get("target_consistent") and row.get("citable", True)
                ...
                and (not row.get("causal_target_required") or row.get("causal_relevance"))
                and len(text) >= min_chars and len(text.split()) >= 12
                and (len(row.get("evidence_dimensions") or []) >= 2 or row.get("direct_target_coverage") and row.get("cause_facet_score", 0) > 0)
...
def coverage_report(selected: list[dict[str, Any]], candidates: list[dict[str, Any]], analysis: CentralQuestionAnalysis, config) -> tuple[bool, dict[str, Any]]:
    ...
    strong_dims = sorted({dim for row in selected if strong_evidence(row, min_chars=config.strong_evidence_min_chars)
                          for dim in dimension_spans(str(row.get("text") or ""), analysis.event if analysis.event_type else None)})
    balance = {}
    ...
    if analysis.administrative_level:
        ...
        direct = [row for row in selected if row.get("direct_target_coverage") and row.get("citable", True)
                  and len(str(row.get("text") or "")) >= config.strong_evidence_min_chars]
        sufficient = any(row.get("causal_relevance") for row in direct) if analysis.question_type == "cause" else bool(direct)
        reason = "direct_relational_target_coverage" if sufficient else "relational_target_or_cause_missing"
    ...
    return sufficient, {
        ...
        **(facet_debug if multi_facet(analysis) else {}),
        ...
        "required_actor_coverage": list(analysis.actors),
        ...
        "unresolved_actor_scopes": [actor for actor in analysis.actors if not any(actor in row.get("actor_scope", []) for row in selected)],
        "direct_target_coverage": any(row.get("direct_target_coverage") for row in selected),
    }
