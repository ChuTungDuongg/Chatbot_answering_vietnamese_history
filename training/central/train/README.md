...
`python -m training.train_qwen3_8b_agent` remains the stable compatibility command. `python -m training.central.train.cli` is equivalent.
...
```bash
python -m training.train_qwen3_8b_agent \
  --config training/central/configs/l4_24gb.json \
  --dataset-root /content/drive/MyDrive/vn-history/central-v2/prepared \
  --drive-root /content/drive/MyDrive/vn-history \
  --run-name central-v2 \
  --learning-rate 8e-5
...
python -m training.train_qwen3_8b_agent --config training/central/configs/l4_24gb.json --dump-config
```
